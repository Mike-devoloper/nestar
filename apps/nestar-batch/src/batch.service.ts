...
import { MemberStatus, MemberType } from 'apps/nestar-api/src/libs/types/enums/member.enums';
import { PropertyStatus } from 'apps/nestar-api/src/libs/types/enums/property.enum';
...
export class NestarBatchService {
  ...
  public async batchTopAgents(): Promise<void> {
    ...
    const promisedList = agents.map(async (ele: Member) => {
      const { _id, memberProperties, memberLikes, memberArticle, memberViews } = ele;
      const rank =
        memberProperties * 4 +
        memberArticle * 3 +
        memberLikes * 2 +
